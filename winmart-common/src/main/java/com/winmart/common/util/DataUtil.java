...
//    }
    ...
    public static Sort getSort(RequestDto requestDto) {
        if (CollectionUtils.isEmpty(requestDto.getSortField())) {
            return null;
        }
        List<Sort.Order> orders = new ArrayList<>();
        for (SortField field : requestDto.getSortField()) {
            if (CommonConstants.SQL_STATEMENT.DESC.equalsIgnoreCase(field.getSort())) {
                orders.add(Sort.Order.desc(field.getFieldName()));
            } else {
                orders.add(Sort.Order.asc(field.getFieldName()));
            }
    ...
    public static Pageable returnPageable(Integer page, Integer size) {
        if (page == null || page <= 0) {
            page = 0;
        } else {
            page = page - 1;
    ...
    public static Pageable returnPageable(RequestDto requestDto) {
        Integer page = requestDto.getPage();
        Integer size = requestDto.getSize();
    ...
    public static String safeToFileName(String name) {
        return name.replaceAll("\\\\|/|:|\\*|\\?|\\\"|<|>|\\|", "__");
    }

    public static boolean regexString(String input, String pattern) {
    ...
    public static BigDecimal convertToBigDecimal(String input) {
        try {
            ...
            return ((BigDecimal) decimalFormat.parse(input)).setScale(3, RoundingMode.HALF_UP);
        } catch (ParseException e) {
